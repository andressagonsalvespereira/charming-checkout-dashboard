...
export const createOrder = async (orderData: any): Promise<Order> => {
  try {
    ...
    return {
      ...
      customer: {
        name: order.customerName || orderData.customer?.name,
        email: order.customerEmail || orderData.customer?.email,
        cpf: order.customerCpf || orderData.customer?.cpf,
        phone: order.customerPhone || orderData.customer?.phone,
